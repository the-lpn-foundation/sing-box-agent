...
func (c *ConfigClient) saveAndReload(_ context.Context, root map[string]interface{}) error {
	...
	// Reload sing-box asynchronously so the HTTP response is sent before
	// systemctl runs. sing-box reload can briefly disrupt TCP connections
	// on the host, causing ECONNRESET for the HTTP client.
	go func() {
		reloadCtx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()

		if err := c.reloadSystemService(reloadCtx); err != nil {
			// Rollback config and retry reload
			if restoreErr := os.WriteFile(c.configPath, previousConfig, 0o644); restoreErr != nil {
				fmt.Fprintf(os.Stderr, "reload failed: %v; rollback write failed: %v\n", err, restoreErr)
				return
			}
			_ = c.reloadSystemService(reloadCtx)
